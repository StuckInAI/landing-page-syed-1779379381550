...
const steps = [
  {
    number: '01',
    icon: <Terminal className="w-6 h-6 text-[#DC2626]" />,
  ...
  {
    number: '02',
    icon: <GitPullRequest className="w-6 h-6 text-[#DC2626]" />,
...
export default function HowItWorks() {
  return (
    <section id="how-it-works" className="py-24 bg-[#1A0E0E]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-[#DC2626]/10 border border-[#DC2626]/20 rounded-full px-4 py-1.5 mb-5">
            <span className="text-xs text-[#DC2626] font-medium uppercase tracking-widest">How it works</span>
          </div>
        ...
        <div className="relative">
          {/* Connector line */}
          <div className="hidden lg:block absolute top-1/2 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#DC2626]/30 to-transparent -translate-y-1/2" />

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            {steps.map((step, i) => (
              <div key={i} className="relative flex flex-col items-center text-center">
                {/* Number badge */}
                <div className="w-16 h-16 rounded-2xl bg-[#221414] border border-white/10 flex items-center justify-center mb-5 relative z-10">
                  <span className="text-2xl font-extrabold text-[#DC2626]/40">{step.number}</span>
                </div>
                {/* Icon */}
